...
2. 文件已存在？
   → 读取现有内容，逐条按语义调和后写入（不做字符串匹配）：
     · 语义重复（同一意图、措辞不同）→ 跳过，不重复添加
     · 细化或抵触已有条目 → 就地替换旧条目，绝不让两条矛盾指令并存
     · 确为新条目 → 追加到对应小节
   → 文件是持续维护的活记录，不是只增日志，每次写入后都保持内部自洽

3. 写入 skill 文件后，自动注册到项目 CLAUDE.md：
...
所有触发都按**语义意图**判断，不做关键词匹配——且**双向成立**：没出现示例词但意图吻合照样触发，
出现了示例词但意图不符则不触发。示例恰好用中英文写，意图本身与语言无关。

满足以下任一条件时激活：

- **会话结尾**：消息传达出**收尾意图**——用户表示今天到此为止、感谢本次协作或示意完成（`"done"` /
  `"谢谢"` / `"收工"` 等只是示例，含义相近的表达同样触发）；或主要任务全部完成、无新需求
- **主动请求**：用户要求把约定/偏好**持久化**为 skill 文件——判断依据是有没有出现"持久化信号"
  （提到文件、skill、记住、保存、以后/下次自动加载等指向对话之外的词），而不是匹配固定短语。
  如果只是普通回顾请求（比如"总结一下你注意到的事项""我们聊了什么"），Claude 会先正常回答，
  只有当回顾内容本身就是约定/偏好时，才会额外问一句要不要顺便存成 skill
...
- **`/compact` 建议**：Claude 准备建议你运行 `/compact` 时，会附加提醒——现在运行
  session-to-skill 能避免这些约定在压缩后丢失
